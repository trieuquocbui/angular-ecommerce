...
export class DeliveredComponent implements OnInit {
  ...
  loadingData(){
    this.orderService.getListDeliveredOrder(this.page!, this.limit!).subscribe((result) => {
  ...
  onPageChange(pageNumber: number) {
    if (pageNumber > 0 &&  pageNumber <= this.totalItems){
      ...
      this.router.navigate(['/admin/delivered'], { queryParams: { page: this.currentPage,limit:this.limit},queryParamsHandling: 'merge' });
    }
  }
