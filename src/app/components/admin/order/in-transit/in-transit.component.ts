...
export class InTransitComponent implements OnInit {
  ...
  loadingData(){
    this.orderService.getListInTransitOrder(this.page!, this.limit!).subscribe((result) => {
  ...
  onPageChange(pageNumber: number) {
    if (pageNumber > 0 &&  pageNumber <= this.totalItems){
      ...
      this.router.navigate(['/admin/in-transit'], { queryParams: { page: this.currentPage,limit:this.limit},queryParamsHandling: 'merge' });
    }
  }
