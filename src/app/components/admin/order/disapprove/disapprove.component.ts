...
export class DisapproveComponent implements OnInit {
  ...
  loadingData(){
    this.orderService.getListDisApprovingOrder(this.page!, this.limit!).subscribe((result) => {
  ...
  onPageChange(pageNumber: number) {
    if (pageNumber > 0 &&  pageNumber <= this.totalItems){
      ...
      this.router.navigate(['/admin/disapprove'], { queryParams: { page: this.currentPage,limit:this.limit},queryParamsHandling: 'merge' });
    }
  }
